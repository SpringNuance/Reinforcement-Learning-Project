...
class DDPGExtension(DDPGAgent):
    def __init__(self, config=None):
        ...
        self.q1 = self.q
        self.q1_target = self.q_target
        self.q1_optim = self.q_optim
    ...
    def _update(self,):
        ...
        # compute target q
        q1_target = batch.reward + self.gamma * self.q1_target(batch.next_state, self.pi_target(batch.next_state)) * batch.not_done
        q2_target = batch.reward + self.gamma * self.q2_target(batch.next_state, self.pi_target(batch.next_state)) * batch.not_done
