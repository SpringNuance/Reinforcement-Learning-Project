...
from .ddpg_utils import Policy, Critic, ReplayBuffer, DynamicModel
...
class DDPGExtension(DDPGAgent):
    def __init__(self, config=None):
        ...
        self.model = DynamicModel(state_dim, self.action_dim).to(self.device)
        self.model_optim = torch.optim.Adam(self.model.parameters(), lr=float(self.lr))
    ...
    def _update(self,):
        ...
        # compute model loss
        model_loss = F.mse_loss(pred_next_state, batch.next_state)
        # optimize the model
